...
#[repr(C)]
pub struct Prepared<'a>(usize, PhantomData<Cell<&'a ()>>);
impl<'a> Prepared<'a> {
    pub unsafe fn new() -> Self {
        Prepared(0xDEADBEEF, PhantomData)
    }
}
...
pub struct AllocScope<'a, T: AllocTracker, ID: EFID>(T, PhantomData<&'a ()>, PhantomData<ID>);
impl<'a, T: AllocTracker, ID: EFID> AllocScope<'a, T, ID> {
    pub unsafe fn new(tracker: T) -> Self {
        AllocScope(tracker, PhantomData, PhantomData)
    }
...
impl<T: 'static> EFPtr<T> {
    ...
    pub unsafe fn upgrade_unchecked<'alloc, ID: EFID>(&self) -> EFRef<'alloc, ID, T> {
        EFRef(
    ...
    pub fn upgrade<'alloc, R: AllocTracker, ID: EFID>(
        ...
        if self.0.is_aligned()
            && alloc_scope
                .tracker()
                .is_valid(self.0 as *const u8, core::mem::size_of::<T>())
        {
    ...
    pub unsafe fn upgrade_unchecked_mut<'alloc, ID: EFID>(&self) -> EFMutRef<'alloc, ID, T> {
        EFMutRef(
            &*(self.0 as *mut UnsafeCell<MaybeUninit<T>> as *const _),
            PhantomData,
        )
    ...
    pub fn upgrade_mut<'alloc, R: AllocTracker, ID: EFID>(
        ...
        if self.0.is_aligned()
            && alloc_scope
                .tracker()
                .is_valid_mut(self.0 as *mut u8, core::mem::size_of::<T>())
        {
    ...
    pub fn upgrade_slice<'alloc, R: AllocTracker, ID: EFID>(
        ...
        if self.0.is_aligned()
            && alloc_scope
                .tracker()
                .is_valid(self.0 as *const u8, length * core::mem::size_of::<T>())
        {
    ...
    pub fn upgrade_slice_mut<'alloc, R: AllocTracker, ID: EFID>(
        ...
        if self.0.is_aligned()
            && alloc_scope
                .tracker()
                .is_valid_mut(self.0 as *mut u8, length * core::mem::size_of::<T>())
        {
...
impl<T: 'static> Clone for EFCopy<T> {
    fn clone(&self) -> Self {
        ...
        clone.as_bytes_mut().copy_from_slice(self.0.as_bytes());
        EFCopy(clone)
    }
}
...
impl<T: 'static> EFCopy<T> {
    ...
    pub fn update_from_ref<ID: EFID>(
        ...
        self.0
            .as_bytes_mut()
            .copy_from_slice((unsafe { &*r.0.get() }).as_bytes());
    }
...
impl<'alloc, ID: EFID, T: 'static> EFMutRef<'alloc, ID, T> {
    pub unsafe fn assume_valid<'access>(
        ...
        EFVal(&*(self.0 as *const _ as *const T), PhantomData, PhantomData)
    }
    ...
    pub unsafe fn sub_ref_unchecked<U: 'static>(
        ...
        EFMutRef(
            unsafe {
                &*((self.0 as *const UnsafeCell<MaybeUninit<T>>).byte_add(byte_offset)
    ...
    pub fn write_copy<'access>(
        ...
        (unsafe { &mut *self.0.get() })
            .as_bytes_mut()
            .copy_from_slice(copy.0.as_bytes())
    }
...
impl<'alloc, const N: usize, ID: EFID, T: 'static> EFMutRef<'alloc, ID, [T; N]> {
    ...
    pub unsafe fn get_unchecked(&self, idx: usize) -> EFMutRef<'alloc, ID, T> {
        EFMutRef(
            &*((self.0 as *const UnsafeCell<MaybeUninit<[T; N]>>
...
impl<'alloc, ID: EFID, T: 'static> EFMutSlice<'alloc, ID, T> {
    ...
    pub fn write_from_iter<'access, I: Iterator<Item = T>>(
        ...
        // memory is mutably accessible and well-aligned.
        let mut count = 0;
        self.0.iter().zip(src).for_each(|(dst, val)| {
        ...
        // instance of T, and we copied self.0.len() elements from `src`:
        unsafe { self.assume_valid(access_scope) }
    }
}
...
#[repr(transparent)]
pub struct EFRef<'alloc, ID: EFID, T: 'static>(&'alloc UnsafeCell<MaybeUninit<T>>, PhantomData<ID>);
...
impl<'alloc, const N: usize, ID: EFID, T: 'static> EFRef<'alloc, ID, [T; N]> {
    ...
    pub fn as_slice(&self) -> EFSlice<'alloc, ID, T> {
        EFSlice(
            unsafe {
                core::slice::from_raw_parts(
                    self.0 as *const _ as *const UnsafeCell<MaybeUninit<T>>,
                    N,
                )
            },
            PhantomData,
...
impl<'alloc, ID: EFID, T: 'static> EFSlice<'alloc, ID, T> {
    ...
    pub fn get(&self, idx: usize) -> Option<EFRef<'alloc, ID, T>> {
        self.0.get(idx).map(|elem| EFRef(elem, PhantomData))
    }
...
pub mod primitives {
	...
	// Generic version, broken currently:
