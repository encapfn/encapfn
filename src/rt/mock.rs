...
enum MockRtAllocError {
    InvalidLayout,
}

trait MockRtAllocator {
    fn with_alloc<R, F: FnOnce(*mut ()) -> R>(&self, layout: core::alloc::Layout, f: F) -> Result<R, MockRtAllocError>;
}
...
unsafe impl<ID: EFID> EncapfnRt for MockRt<ID> {
    type ID = ID;
    ...
    where
        F: FnOnce(*mut u8) -> R,
    {
        use crate::util::stack_alloc::with_stacked_alloc;

        if layout.size() == 0 {
            return Err(EFError::AllocInvalidLayout);
        }

        with_stacked_alloc(layout.size(), layout.align(), |ptr| {
            // Invoke the passed function. No need to deallocate memory,
            // returning from this function will pop the stack frame.
            Ok(fun(ptr as *mut u8))
        })
    }
